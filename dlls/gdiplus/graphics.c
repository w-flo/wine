...
static ARGB blend_colors_premult(ARGB start, ARGB end, REAL position)
{
    ...
    return (final_a >> 8) << 24 |
        ((((start >> 16) & 0xff) * start_weight + (((end >> 16) & 0xff) * end_weight)) >> 8) << 16 |
        ((((start >> 8) & 0xff) * start_weight + (((end >> 8) & 0xff) * end_weight)) >> 8) << 8 |
        (((start & 0xff) * start_weight + ((end & 0xff) * end_weight)) >> 8);
}
...
{
    struct sample_point *points = heap_alloc_zero(sizeof(struct sample_point) * dst_size * 2);
    int i;

    for (i = 0; points && i < dst_size; i++)
    {
...
static struct sample_point* calculate_sampling_nearest(int src_start, int src_size, int dst_start, int dst_size, float base_pos, REAL delta, BOOL tiling, BOOL flip)
{
    ...
    struct sample_point *points = heap_alloc_zero(sizeof(struct sample_point) * dst_size);
    int i;
    ...
    GpGraphics *graphics, GDIPCONST GpImageAttributes *attributes)
{
    ...
    tmp_data = heap_alloc_zero(sizeof(ARGB) * dst_area.Width * samples_per_pixel);
    ...
    for (dst_y = 0; dst_y < dst_area.Height; dst_y++)
    {
        ...
        for (sample_y = 0; sample_y < samples_per_pixel; sample_y++)
        {
            ...
            for (dst_x = 0; dst_x < dst_area.Width; dst_x++)
            {
                ...
                *tmp_pixel = ((a >> PRECISION) << 24) | ((r >> PRECISION) << 16) | ((g >> PRECISION) << 8) | (b >> PRECISION);
            }
            tmp_rows_y[sample_y] = point_y.offset;
        }
        ...
        for (dst_x = 0; dst_x < dst_area.Width; dst_x++)
        {
            ...
            *dst_pixel = ((a >> PRECISION) << 24) | ((r >> PRECISION) << 16) | ((g >> PRECISION) << 8) | (b >> PRECISION);
        }
    }

end:
    heap_free(sample_points_horiz);
    heap_free(sample_points_vert);
    heap_free(tmp_data);
}
